...
const Faucet = () => {
  ...
  const { services, hasAnySuccess } = useFaucet();
              ...
              <div className="flex items-start space-x-4 p-6 bg-muted/30 rounded-xl border border-border/50 hover:border-primary/30 transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Claim free testnet CORE and BTC tokens from the official
                    Core Testnet faucet. You'll need these to test all platform
                    features.
                  </p>
                  <div className="flex flex-wrap gap-3">
                    ...
                    {services.map((service) => (
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        onClick={service.mint}
                        disabled={
                          !isConnected || service.isLoading || !service.canMint
                        }
                        className={`group ${service.color.bg} ${service.color.border} ${service.color.text} ${service.color.hover}`}
                      >
                        {service.isLoading ? (
                          <>
                        ...
                        ) : !service.canMint && service.timeRemaining ? (
                          <>
                            <Clock className="w-4 h-4 mr-2" />
                            {service.formatTimeRemaining(service.timeRemaining)}
                          </>
                        ) : (
                          <>
                            {service.symbol === "wBTC" ? (
                              <BitcoinIcon size="sm" className="mr-2" />
                            ) : (
                              <CoreIcon size="sm" className="mr-2" />
                            )}
                            Mint {service.symbol} ({service.amount})
                          </>
                        )}
                      </Button>
                    ))}

                    {/* External stCORE Mint Button */}
                    <Button
                      asChild
                      size="sm"
                      variant="outline"
                      className="group bg-blue-500/10 border-blue-500/20 text-blue-600 hover:bg-blue-500/20"
                    >
                      <a
                        href="https://stake.test2.btcs.network/liquid-staking/stcore"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <CoreIcon size="sm" className="mr-2" />
                  ...
                  {isConnected &&
                    services.some(
                      (service) => !service.canMint && service.timeRemaining
                    ) && (
                      <div className="mt-3 space-y-2">
                        {services
                          ...
                          .map((service) => (
                            <div
                              key={service.symbol}
                              className={`p-2 ${service.color.bg} border ${service.color.border} rounded-lg`}
                            >
                              <p
                                className={`text-sm ${service.color.text} flex items-center`}
                              >
                                <Clock className="w-4 h-4 mr-2" />
                                Next {service.symbol} mint available in{" "}
                                {service.formatTimeRemaining(
                  ...
                  {hasAnySuccess && (
                    <div className="mt-3 space-y-2">
                      {services
                        .filter((service) => service.isSuccess)
                        .map((service) => (
                          <div
                            key={service.symbol}
                          ...
                          >
                            <p className="text-sm text-green-600 flex items-center">
                              ...
                              Successfully minted {service.amount}{" "}
                              {service.symbol} to your wallet!
                            </p>
                          </div>
                        ))}
                    </div>
          ...
          <Card className="bg-gradient-to-br from-primary/5 to-primary-glow/5 border-primary/20">
            <CardContent className="p-6">
              ...
              <p className="text-sm text-muted-foreground">
                All testnet activities are completely safe. No real assets are
                at risk, and you can experiment freely with all platform
                features.
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-green-500/5 to-emerald-500/5 border-green-500/20">
            <CardContent className="p-6">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                platform, check our documentation or reach out to our support
                team.
              </p>
            </CardContent>
          </Card>
